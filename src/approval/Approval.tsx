...
import type { ApprovalQueueItem, HodProgrammeSlot } from "@/lib/requisitionsApi";
import { fetchApprovalDetail, fetchApprovalQueue, submitApproval } from "@/lib/requisitionsApi";
import {
  formatHistoryDate,
...
function ApprovalDialog({
  ...
  const { data: item, isLoading, isError, error } = useQuery({
    ...
    queryFn: () => fetchApprovalDetail(requisitionId!),
              ...
              {item.hrVerification ? (
                <DetailSection title="HR verification">
                  <div className="grid gap-3 sm:grid-cols-2">
                    <DetailField label="Verified by" value={item.hrVerification.name} />
                    <DetailField label="HR email" value={item.hrVerification.email} />
                    <DetailField label="Verified on" value={formatHistoryDate(item.hrVerification.verifiedAt)} />
                    {item.hrVerification.remarks ? (
                      <div className="sm:col-span-2">
                        <DetailField label="Remarks" value={item.hrVerification.remarks} />
                      </div>
                    ) : null}
                  </div>
                </DetailSection>
...
export function ApprovalQueuePage() {
  ...
  const [confirmRejectItem, setConfirmRejectItem] = React.useState<ApprovalQueueItem | null>(null);
  const [rejectRemarks, setRejectRemarks] = React.useState("");

  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["requisitions", "approval", "queue"],
  ...
  const approvalMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["requisitions", "approval", "queue"] });
      queryClient.invalidateQueries({ queryKey: ["requisitions", "approval", "detail"] });
          ...
          <div className="container mx-auto py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h1 className="font-display text-2xl font-bold tracking-tight">Approval</h1>
              </div>
            </div>
          </div>
        </header>
        ...
        <div className="container mx-auto py-8">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Requisitions verified by HR and pending dean approval.
              </CardDescription>
            </CardHeader>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="hidden md:table-cell">HR verified</TableHead>
                      ...
                      rows.map((row) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell className="hidden md:table-cell">
                            {row.hrVerification ? (
                              <div className="grid gap-0.5">
                                <p>{row.hrVerification.name}</p>
                                <p className="text-xs text-muted-foreground">
                                  {formatHistoryDate(row.hrVerification.verifiedAt)}
                                </p>
                              </div>
                            ) : (
                              "—"
      ...
      >
        <AlertDialogContent>
          ...
          <div className="grid gap-2 py-2">
            <Label htmlFor="approvalRejectRemarks">Rejection remarks</Label>
            <Textarea
              id="approvalRejectRemarks"
